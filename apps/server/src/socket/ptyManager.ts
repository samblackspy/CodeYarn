...
export async function startPtySessionAndAttachSocket(
  initialSocket: Socket,
  containerId: string
) {
  console.log(
    `[PTY] Attempting to start PTY session for container ${containerId} by socket ${initialSocket.id}`
  );
  try {
    ...
    console.log(
      `[PTY] Stream started successfully for ${containerId}. Initial socket: ${initialSocket.id}`
    );
    ...
    console.log(
      `[PTY] Emitted 'terminal-ready' to initial socket ${initialSocket.id} for ${containerId}`
    );
  } catch (error: any) {
    console.error(
      `[PTY] Failed to start PTY session for container ${containerId}:`,
      error
    );
    initialSocket.emit("terminal-error", {
