...
export async function findFullContainerId(
  partialId: string
): Promise<string | null> {
  try {
    if (partialId.length > 12) {
      // Assuming Docker short IDs are 12 chars
    ...
    const containerRecord = allContainers.find((c: { id: string }) =>
    ...
    if (partialId.length > 12) {
      console.warn(
        `[ContainerService] Partial ID ${partialId} looks like a full ID but not found in DB via prefix search.`
      );
    ...
    console.error(
      `[API Internal] Error finding full container ID for ${partialId}:`,
      error
    );
    return null;
  }
