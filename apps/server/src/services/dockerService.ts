...
export async function getContainerSafely(containerId: string): Promise<Docker.Container | null> {
    try {
        ...
        console.error(`[DockerService] Error inspecting container ${containerId}:`, error);
        throw error;
    }
}
...
export async function execCmdInContainer(
    ...
    console.log(`[DockerService] Running in ${container.id}: ${cmd.join(' ')} at ${workingDir}`);
    ...
    try {
        ...
        if (inspectData.ExitCode !== 0) {
            console.error(`[DockerService] Command "${cmd.join(' ')}" failed. Code: ${inspectData.ExitCode}. Stderr: ${execError.trim()}`);
            return { success: false, stdout: execOutput.trim(), stderr: execError.trim() };
        }
        return { success: true, stdout: execOutput.trim(), stderr: execError.trim() };
    } catch (error: any) {
        console.error(`[DockerService] Error in execCmd "${cmd.join(' ')}":`, error.message);
        return { success: false, stdout: execOutput.trim(), stderr: execError.trim() || error.message };
    }
}